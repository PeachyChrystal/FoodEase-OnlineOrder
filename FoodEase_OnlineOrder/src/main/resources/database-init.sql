...
(
--     如果是primary key的话不用谢NOT NULL，因为默认就是NOT NULL
    id         SERIAL PRIMARY KEY   NOT NULL,
--     其实这里email更好地是用CITEXT，表示大小写不重要：eddy@laioffer.com和EDDY@laioffer.com是一样的
    email      TEXT UNIQUE          NOT NULL,
    enabled    BOOLEAN DEFAULT TRUE NOT NULL,
    password   TEXT                 NOT NULL,
--     没有NOT NULL就可以是空的，不强制提供first_name和last_name
    first_name TEXT,
    last_name  TEXT
);
...
-- ON DELETE SET NULL的意思是如果某个id的record被删除了，那么customer_id将变成NULL，不过注意那么customer_id将变成NULL
-- 本身NOT NULL了就不能SET NULL了
-- ON DELETE CASCADE的话如果某个id的record被删除了，那么customer_id的record在carts中环直接被删除
-- foreign key可以建立索引，有一个搜索树，那么到拿着一个customer_id到customer里面找对应的时候time就是log(n)
